...
export class WebinarHonorariumRequestComponent implements OnInit {
      ...
      eventList.forEach(event => {
        ...
        if (event.EventType == 'Webinar') {
          if (event['Initiator Email'] == this._userDetails.email) {
            if (event['Event Request Status'] == 'Approved' && (event['Event Request Status'].indexOf('Waiting') > -1 || (event['Event Request Status'].indexOf('Rejected')))) {
  ...
  filterHonor(eventId: any) {
    ...
    console.log('honorsnihuedec', this.honortableDetails);
          ...
          reader.onloadend = () => {
            ...
            if (type == 'deviation') {
              ...
              if (control == 'uploadAttendecnce') {
                this.uploadAttendenceFile = base64String.split(',')[1]
              }
  ...
  private _resetControl(control: string) {
    switch (control) {
      ...
      case 'uploadAttendecnce':
        this.honorarDetails.controls.uploadAttendecnce.reset();
    }
  }
  ...
  onSubmit() {
    ...
    if (formValidity == 0) {
      ...
      if (this.honortableDetails.length > 0) {
        ...
        this.otherFiles = this.photosFiles.concat(this.honorariumInvoiceFiles).concat(this.speakerAggrementFiles).concat(this.uploadAttendenceFile);
        ...
        const reqHonor = {
          ...
          TotalAccomodationSpend: (currentEventDetails['Total Accomodation Amount']) ? currentEventDetails['Total Accomodation Amount'] + '' : '0',
