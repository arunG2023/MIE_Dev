...
export class WebinarPostEventSettlementComponent implements OnInit {
  ...
  // Amount Table 
  panelSelectionFinalAmount: number = 0;
  inviteesFinalAmount: number = 0;
  expenseFinalAmount: number = 0;
  amountTableFinalAmount: number = 0;
  ...
  addToExpenseTable() {
    ...
    if (this.addExpenseForm.valid && this.addExpenseForm.value.expenseAmountWithoutGST > 0) {
      ...
      const expense = {
        Amount: (this.addExpenseForm.value.expenseAmountWithoutGST + this.addExpenseForm.value.gstAmount) + '0',
